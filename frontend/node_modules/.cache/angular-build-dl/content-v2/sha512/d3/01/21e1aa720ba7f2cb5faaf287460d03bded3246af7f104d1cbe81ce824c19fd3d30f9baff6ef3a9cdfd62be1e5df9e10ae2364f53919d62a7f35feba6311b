          ...
          value: function getMenu(i) {
            ...
            this.httpService.getMenu().subscribe(function (res) {
              ...
              if (res.status === 200) {
                ...
                res.menu.forEach(function (m) {
                  _this3.menu_list.push({
                    symbol: m.symbol,
                    title: m.title.uz
                  });
                });
                _this3.loading = false; // localStorage.setItem("menu", JSON.stringify(res.menu));
              } else {
                alert("Bazaka ulana olmadim yana bir bor urinib ko‘ring!");
                window.location.reload();
              }
