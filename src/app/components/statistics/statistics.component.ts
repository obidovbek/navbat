...
import { PvoKafInter, FieldData, Chart } from 'src/app/shared/interface/field.interface'
...
export class StatisticsComponent {
	...
	genTable(data){
		...
		Object.keys(new_data).map(item=>{
			this.statistics_chart.data.labels.push(item);
			// services_used_ng2.push({service_name: item, times_});
			services_used.push(new_data[item]);
		})
		this.statistics_chart.data.series.push(services_used);
		this.tmp.loading = false;
